...
export const updateUserScore = (userId) => async (dispatch, getState) => {
  ...
  const user = {
    ...
    const { data } = await axios.put(`${BASE_URL}/api/users/score`, user, config);
    dispatch({ type: USER_UPDATE_SCORE_SUCCESS, payload: data });
  } catch (error) {
    dispatch({
